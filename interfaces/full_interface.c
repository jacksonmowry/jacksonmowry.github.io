...
HitRecord square_hit(Shape* self, double aspect_ratio, double x, double y) {
    ...
    SquareState* s = (SquareState*)self->state;

    HitRecord hr = {0};

    double x1 = s->x - s->half_side_length * aspect_ratio;
    double x2 = s->x + s->half_side_length * aspect_ratio;
    double y1 = s->y - s->half_side_length;
    double y2 = s->y + s->half_side_length;
...
HitRecord circle_hit(Shape* self, double aspect_ratio, double x, double y) {
    ...
    CircleState* s = (CircleState*)self->state;

    // Uses the elipse collision formula
    // \frac{(x-h)^2}{r_x^2} + \frac{(y-k)^2}{r_y^2} \leq 1
    double lhs = (pow(x - s->x, 2) / pow(s->radius * aspect_ratio, 2)) +
                 (pow(y - s->y, 2) / pow(s->radius, 2));
...
void circle_deinit(Shape* self) { free(self->state); }
