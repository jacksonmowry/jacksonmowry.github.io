...
int add_accept_request(int listen_socket, struct sockaddr_in* client_addr,
    ...
    struct request* listening_request = malloc(sizeof(struct request));
...
int add_write_request(struct request* req) {
    ...
    size_t header_length =
        snprintf(req->buffer, BUFFER_SIZE,
                 ...
                 "text/plain\r\nContent-Length: %ld\r\n\r\n",
                 st.st_size);
    code = send(files[req->socket], req->buffer, header_length, 0);
    if (code < 0) {
        perror("send");
        free(final_path);
    ...
    free(final_path);
    code = sendfile(files[req->socket], fd, NULL, st.st_size);
    if (code < 0) {
        perror("sendfile");
        struct io_uring_sqe* sqe = io_uring_get_sqe(&ring);
        req->type = CLOSE;
...
int add_close_request(struct request* req) {
    struct io_uring_sqe* sqe = io_uring_get_sqe(&ring);
    req->type = CLOSE;
    io_uring_prep_close_direct(sqe, req->socket);
    io_uring_sqe_set_data(sqe, req);
    /* io_uring_submit(&ring); */
...
int main() {
    ...
    int err = io_uring_register_files(&ring, files, MAX_FILES);
    if (err < 0) {
