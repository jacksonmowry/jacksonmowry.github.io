...
#define DIE_ERRNO(err, ...)                                                    \
    do {                                                                       \
        error_at_line(EXIT_FAILURE, errno, __FILE__, __LINE__,                 \
                      err __VA_OPT__(, ) __VA_ARGS__);                         \
    } while (0)

#define DIE_STRERR(err, errno, ...)                                            \
    do {                                                                       \
        error_at_line(EXIT_FAILURE, errno, __FILE__, __LINE__,                 \
                      err __VA_OPT__(, ) __VA_ARGS__);                         \
    } while (0)
...
struct request_data {
    enum { ACCEPT, RECV, SEND, CLOSE } type;
    int fd;
    int buf_id;
};
...
void setup_buffer_ring() {
    ...
    if ((ret = posix_memalign((void**)&br, 4096,
                              BUFS_IN_GROUP *
                                  sizeof(struct io_uring_buf_ring))) != 0)
...
void submit_multishot_accept(int serversock) {
    ...
    io_uring_prep_multishot_accept(sqe, serversock,
                                   (struct sockaddr*)&echoclient, &socklen, 0);
    ...
    while (1) {
        ...
        io_uring_for_each_cqe(&ring, head, cqe) {
            ...
            case SEND:
                ...
                io_uring_buf_ring_add(br, bufs[cqe_data->buf_id], BUFSZ, 1,
                                      io_uring_buf_ring_mask(BUFS_IN_GROUP),
                                      cqe_data->buf_id);
