...
pub fn printStruct(s: anytype, shorten_types: bool, indent: comptime_int) void {
    ...
    inline for (s_type_info.Struct.fields) |fld|
        switch (@typeInfo(fld.type)) {
            .Int, .Float, .ComptimeInt, .ComptimeFloat => std.debug.print("{s}{s}{s}: {s} = {d}\n", .{ ind_str_2, if (fld.is_comptime) "comptime " else "", fld.name, @typeName(fld.type), @field(s, fld.name) }),
            .Bool => std.debug.print("{s}{s}{s}: {s} = {}\n", .{ ind_str_2, if (fld.is_comptime) "comptime " else "", fld.name, @typeName(fld.type), @field(s, fld.name) }),
            .Struct => {
                std.debug.print("{s}{s}{s}: {s} =\n", .{ ind_str_2, if (fld.is_comptime) "comptime " else "", fld.name, typeName(@typeName(fld.type), &name_buf, shorten_types) });
                printStruct(@field(s, fld.name), shorten_types, indent + 1);
            },
            .Array => {
                std.debug.print("{s}{s}{s}: {s} = [ ", .{ ind_str_2, if (fld.is_comptime) "comptime " else "", fld.name, typeName(@typeName(fld.type), &name_buf, shorten_types) });
                printArray(@field(s, fld.name), shorten_types);
                std.debug.print("]\n", .{});
            },
            .Pointer => |ptr_type_info| {
                switch (ptr_type_info.size) {
                    .One, .Many, .C => std.debug.print("{s}{s}{s}: {s} = {*}\n", .{ ind_str_2, if (fld.is_comptime) "comptime " else "", fld.name, typeName(@typeName(fld.type), &name_buf, shorten_types), @field(s, fld.name) }),
            ...
            .Enum => std.debug.print("{s}{s}{s}: {s} = {s}\n", .{ ind_str_2, if (fld.is_comptime) "comptime " else "", fld.name, typeName(@typeName(fld.type), &name_buf, shorten_types), @tagName(@field(s, fld.name)) }),
            else => std.debug.print("{s}{s}{s}: {s} = -\n", .{ ind_str_2, if (fld.is_comptime) "comptime " else "", fld.name, typeName(@typeName(fld.type), &name_buf, shorten_types) }),
        };
    std.debug.print("{s}}}\n", .{ind_str});
}

fn printStructInline(s: anytype, shorten_types: bool) void {
    const s_type_info = @typeInfo(@TypeOf(s));
    if (s_type_info != .Struct)
    ...
    inline for (s_type_info.Struct.fields) |fld|
        switch (@typeInfo(fld.type)) {
            .Int, .Float, .ComptimeInt, .ComptimeFloat => std.debug.print("{s}{s}: {s} = {d}, ", .{ if (fld.is_comptime) "comptime " else "", fld.name, @typeName(fld.type), @field(s, fld.name) }),
            .Bool => std.debug.print("{s}{s}: {s} = {}, ", .{ if (fld.is_comptime) "comptime " else "", fld.name, @typeName(fld.type), @field(s, fld.name) }),
            .Struct => {
                std.debug.print("{s}{s}: {s} = ", .{ if (fld.is_comptime) "comptime " else "", fld.name, typeName(@typeName(fld.type), &name_buf, shorten_types) });
            ...
            .Pointer => |ptr_type_info| {
                switch (ptr_type_info.size) {
                    .One, .Many, .C => std.debug.print("{s}{s}: {s} = {*}, ", .{ if (fld.is_comptime) "comptime " else "", fld.name, typeName(@typeName(fld.type), &name_buf, shorten_types), @field(s, fld.name) }),
                    .Slice => if (ptr_type_info.child == u8)
                        std.debug.print("{s}{s}: {s} = \"{s}\", ", .{ if (fld.is_comptime) "comptime " else "", fld.name, typeName(@typeName(fld.type), &name_buf, shorten_types), @field(s, fld.name) })
                    else {
                        std.debug.print("{s}{s}: {s} = [ ", .{ if (fld.is_comptime) "comptime " else "", fld.name, typeName(@typeName(fld.type), &name_buf, shorten_types) });
                        printArray(@field(s, fld.name), shorten_types);
                        std.debug.print("], ", .{});
            ...
            .Enum => std.debug.print("{s}{s}: {s} = {s}, ", .{ if (fld.is_comptime) "comptime " else "", fld.name, typeName(@typeName(fld.type), &name_buf, shorten_types), @tagName(@field(s, fld.name)) }),
            else => std.debug.print("{s}{s}: {s} = -, ", .{ if (fld.is_comptime) "comptime " else "", fld.name, typeName(@typeName(fld.type), &name_buf, shorten_types) }),
        };
    std.debug.print("}}, ", .{});
}
...
fn printArray(a: anytype, shorten_types: bool) void {
    ...
    for (a) |e|
        switch (@typeInfo(@TypeOf(e))) {
            .Int, .Float, .ComptimeInt, .ComptimeFloat => std.debug.print("{d}, ", .{e}),
...
fn typeName(name: []const u8, name_buf: []u8, shorten_name: bool) []const u8 {
    ...
    if (name.len > 0 and name[name.len - 1] == ')') {
        ...
        first_free_name_buf_idx = copyToBufAt(name_buf, name[fn_first_char_after_dot_idx..(open_bracket_idx + 1)], first_free_name_buf_idx);
        ...
        {
            first_free_name_buf_idx = copyToBufAt(name_buf, name[(open_bracket_idx + dot_pos + 1)..], first_free_name_buf_idx);
        } else // no dot
        first_free_name_buf_idx = copyToBufAt(name_buf, name[(open_bracket_idx + 1)..], first_free_name_buf_idx);
    ...
    if (name[0] < 'A' or (name[0] > 'Z' and name[0] < 'a') or name[0] > 'z') {
        ...
        {
            first_free_name_buf_idx = copyToBufAt(name_buf, name[(first_letter_idx + dot_pos + 1)..], first_free_name_buf_idx);
        } else // no dot
        first_free_name_buf_idx = copyToBufAt(name_buf, name[(first_letter_idx)..], first_free_name_buf_idx);
        return name_buf[0..first_free_name_buf_idx];
    }
