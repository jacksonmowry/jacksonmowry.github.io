...
#define R(...)                                                                 \
  string(" " #__VA_ARGS__ " ") // evil stringification macro, similar syntax to
                               // raw string R"(...)"
...
string get_opencl_c_code() {
  ...
  r = replace(r, "#ifdef\n",
              "#ifdef "); // except for the arguments after some preprocessor
                          // options that need to be in the same line
  r = replace(r, "#ifndef\n", "#ifndef ");
  r = replace(r, "#define\n",
...
// everything below is just for syntax highlighting in the editor, this does not
// change any functionality full catalogue:
...
#define barrier(x) // barrier for local work group, x is CLK_LOCAL_MEM_FENCE or
                   // CLK_GLOBAL_MEM_FENCE
#define mem_fence(x) // orders loads/stores, x is CLK_LOCAL_MEM_FENCE or
                     // CLK_GLOBAL_MEM_FENCE
#define read_mem_fence(                                                        \
    x) // orders loads, x is CLK_LOCAL_MEM_FENCE or CLK_GLOBAL_MEM_FENCE
#define write_mem_fence(                                                       \
    x) // orders stores, x is CLK_LOCAL_MEM_FENCE or CLK_GLOBAL_MEM_FENCE
...
// 64-bit integer atomics (cl_khr_int64_base_atomics extension must be supported
// by the device)
...
// integer functions
#define abs(x)           // |x|
#define clz(x)           // count leading 0 bits, slow, instead use
                         // as_uint((float)((x&0x07FF)<<12))>>23
