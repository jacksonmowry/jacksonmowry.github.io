...
inline float as_float(const uint x) { return *(float *)&x; }
inline uint as_uint(const float x) { return *(uint *)&x; }
inline double as_double(const ulong x) { return *(double *)&x; }
inline ulong as_ulong(const double x) { return *(ulong *)&x; }
...
to_string(double x) { // convert double to string with full precision (<string>
  ...
  return s + to_string(integral) + "." + decimal_to_string_double(decimal, 16) +
         (exponent != 0 ? "E" + to_string(exponent) : "");
}
inline string to_string(float x,
  ...
  return s + to_string(integral) +
         (decimals == 0u
              ? ""
              : "." + decimal_to_string_float(decimal, min((int)decimals, 8)));
}
...
inline void print(const string &s = "") { std::cerr << s; }
inline void println(const string &s = "") { std::cerr << s + "\n"; }
inline void reprint(const string &s = "") { std::cerr << "\r" + s; }
...
inline void print_message(const string &message, const string &keyword = "",
  ...
  for (uint i = 0u; i < (uint)v.size(); i++) {
    ...
    if (l <= w + 1u) {    // word fits -> append word and space
      ...
      i--;    // reuse same vector element for overflowing part, decrement i to
              // start next line with this overflowing part
      l = 0u; // reset line length
    } else {  // word does not fit -> fill remaining line with spaces
      l = l - length(v.at(i--)) - 1u; // remove word from line, decrement i to
...
inline float to_float(const string &s, const float default_value) {
  const string t = trim(s);
  return parse_sanity_check(t, "[+-]?(((\\d+\\.?\\d*|\\.\\d+)([eE][+-]?\\d+[fF]"
                               "?)?)|(\\d+\\.\\d*|\\.\\d+)[fF]?)")
             ? (float)atof(t.c_str())
             : default_value;
}
...
inline string create_file_extension(const string &filename,
  ...
  return filename.substr(0, filename.rfind('.')) +
         (extension.at(0) != '.' ? "." : "") +
         extension; // remove existing file extension if existing and replace it
                    // with new one
}
