...
3. The next 48 byte allocation is coming from [[https://linux.die.net/man/3/asprintf][~asprintf()~]]. If you're not familiar with this function I think of it as a bit of a cheat code for strings in C. The function will allocate a buffer to hold the entire string it generates, and gives you the pointer to that string, which you are responsible to free.
...
#  LocalWords:  sbrk size_t malloc hashmap nop NDEBUG NDEBUG LocalWords clangd
#  LocalWords:  glibc seg seg gdb gdb gdb glibc gdb gdb gdb strace hashmap prev
#  LocalWords:  realloc realloc realloc memset calloc calloc glibc calloc glibc
#  LocalWords:  memset calloc calloc hashmaps calloc asprintf

# Local Variables:
# languagetool-local-disabled-rules: ("EXTREME_ADJECTIVES" "A_INFINITIVE")
# languagetool-suggestion_level: picky
# End:
