...
Earlier we discussed the concept of load factor, and we would ideally like to keep this as low as possible (and below 50% in the case of quadratic probing). We wouldn't be able to acomplish this without dynamically allocating memory, so that is exatly what we have to do. However, we should probably keep a few things in mind when decided how our hashmap should behave.

*** Initial Size
If a user is going to instantiate a hashmap is is likely they are going to want to insert elements straight away. So we should either set a sensible default size, or allow this value to be specific at create time. They both sound reasonable, so why not both.
