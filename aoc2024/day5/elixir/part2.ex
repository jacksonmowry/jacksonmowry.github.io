defmodule Day5 do
  ...
  def reorder(print_order, map) do
    print_order
    |> String.split(",")
    |> Enum.reverse()
    |> Enum.scan([], fn x, acc -> [x | acc] end)
    ...
    |> Enum.map(fn x ->
      Enum.map(x, fn w ->
        case map[w] do
          nil ->
            {String.to_integer(w), 0}

          trailing ->
            {String.to_integer(w),
             Enum.count(Enum.map(x, fn y -> contains(trailing, y) end), fn z -> z == true end)}
        end
      end)
    end)
    |> Enum.at(0)
    ...
    |> Enum.sort(fn x, y ->
      contains(Map.get(map, elem(x, 0), []), elem(y, 0))
    end)
    |> Enum.reverse()
  end
end
...
IO.inspect(
  ...
  |> Enum.map(fn x -> Day5.reorder(x, ex_map) end)
  |> Enum.map(fn x -> Enum.map(x, fn y -> elem(y, 0) end) end)
