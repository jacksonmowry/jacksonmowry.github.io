...
=io_uring= allows the program to avoid system calls upon submission, meaning that a system can work entirely in userspace, without the need for costly system calls. This reduces the number of system calls from at a minimum 4 (accept, read, write, close) in a synchronous server, to a potential 0[fn:2] in an =io_uring= based approach.
...
* Footnotes
[fn:2] Once =sendfile= is implemented this will be possible, for now 1 call to =pipe=, and 2 calls to =fcntl= are required to mimic the behavior.
