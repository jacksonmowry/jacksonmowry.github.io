...
void *malloc(size_t bytes) {
  ...
  size_t increment = (bytes > HEAP_INC) ?
    sizeof(heap_seg) + ((bytes + (sizeof(void*)-1)) / sizeof(void*)) * sizeof(void*)
    : HEAP_INC;
  sbrk(increment);
  heap_end = sbrk(0);
  size_t new_size = sizeof(heap_seg) + ((bytes+(sizeof(void*)-1))/sizeof(void*))*sizeof(void*)|0x1;
  ptr->size = new_size;
  ptr->prev = prev;
