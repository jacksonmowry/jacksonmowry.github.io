...
int main() {
  ...
  c_buffer =
      clCreateBuffer(context, CL_MEM_WRITE_ONLY, sizeof(c_mat), NULL, &err);
  ...
  err = clEnqueueNDRangeKernel(queue, transpose_kernel, 1, NULL, &global_size,
                               NULL, 0, NULL, NULL);
  if (err < 0) {
  ...
  err = clEnqueueNDRangeKernel(queue, mult_kernel, 1, NULL, &global_size, NULL,
                               0, NULL, NULL);
  if (err < 0) {
  ...
  /* Read output buffer */
  err = clEnqueueReadBuffer(queue, c_buffer, CL_TRUE, 0, sizeof(c_mat), c_mat,
                            0, NULL, NULL);
  if (err < 0) {
