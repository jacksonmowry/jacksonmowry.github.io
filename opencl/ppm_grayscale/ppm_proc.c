...
#include <stdarg.h>
#define PROGRAM_FILE "ppm_proc.cl"
#define KERNEL_FUNC "ppm_proc"
...
int main() {
  ...
  cl_init_kernel(&ccw, PROGRAM_FILE, KERNEL_FUNC, CL_DEVICE_TYPE_DEFAULT, 4);
  ...
  uint8_t *result = calloc(width * height, 1);

  for (int i = 0; i < height; i++) {
    for (int j = 0; j < width; j++) {
      scanf("%hhd %hhd %hhd", &r[(i * width) + j], &g[(i * width) + j],
            &b[(i * width) + j]);
    }
  }

  bool bound = cl_create_and_bind(&ccw, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR,
                                  sizeof(uint8_t) * width * height, r, 0);
  bound &= cl_create_and_bind(&ccw, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR,
                              sizeof(uint8_t) * width * height, g, 1);
  bound &= cl_create_and_bind(&ccw, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR,
                              sizeof(uint8_t) * width * height, b, 2);
  bound &= cl_create_and_bind(&ccw, CL_MEM_WRITE_ONLY | CL_MEM_COPY_HOST_PTR,
                              sizeof(uint8_t) * width * height, result, 3);
  if (!bound) {
    perror("Failed to bind kernel args");
    return 1;
  }

  ccw.work_units_per_kernel = width * height;

  clEnqueueNDRangeKernel(ccw.queue, ccw.kernel, 1, NULL,
                         &ccw.work_units_per_kernel, NULL, 0, NULL, NULL);

  clEnqueueReadBuffer(ccw.queue, ccw.kernel_args[3], CL_TRUE, 0,
                      sizeof(uint8_t) * width * height, result, 0, NULL, NULL);
