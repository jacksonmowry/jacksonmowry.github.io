...
\label{sec:org6936955}
Web servers are a class of software that sit between nearly every interaction a client makes on their device, and access to the files hosted on a server. Despite this fact their implementations mainly use software designs of the past, due to the fact that for most use cases they are ``good enough''. It is true that even a naive HTTP server can serve thousands of requests per second \cslcitation{1}{[1]}, \cslcitation{2}{[2]}, which easily surpasses the needs of most users, but when it comes time to scale up the same architecture only has two ways to scale. The server can either be rewritten to take advantage of multiple CPU cores, or it can have multiple instances running behind a load balancer \cslcitation{3}{[3]}.

The root of this issue lies in the fact that these workloads are still inhrently synchronous at some level  (excluding \texttt{aio}), which leads to large requests stalling the server. \texttt{io\_uring} brings a true asynchronous io system to the table with its introduction into the Linux kernel (version 5.1) \cslcitation{4}{[4]}, \cslcitation{5}{[5]}. An exciting new space for high performance single core servers now exists, in this paper we will see how \texttt{io\_uring} can fill that gap. To date \texttt{io\_uring} has not been implemented in mainstream web server applications, this paper serves to illuminate the potential benefits, to both speed and latency, of the latest in asynchronous io technology.
\section{Previous Work}
\label{sec:orgbf63d94}
Previous work on maximizing throughput on asynchronous or multiplexed architectures has often focused on \texttt{epoll} as the main differentiating factor\cslcitation{2}{[2]}, \cslcitation{3}{[3]}. The performance of \texttt{epoll} is largely better than that of synchronous mechanisms like \texttt{select} or \texttt{poll}, but \texttt{epoll} alone is not enough to meet modern demands \cslcitation{6}{[6]}. Instead, web servers often implement complex thread pools to handle massively concurrent workloads. By spreading the load out to multiple physical cores, the disadvantages of blocking system calls can be mitigated from a request latency prespective \cslcitation{3}{[3]}.

\texttt{io\_uring} has begun to recieve attention in the storage performance world, where the consistent interface allows for high performace systems across all types of file descriptors. This interface allows for performance critical application to both improve performance, and simplify the event handling code, while keeping the important logic entirely in user-space \cslcitation{7}{[7]}, \cslcitation{8}{[8]}, \cslcitation{9}{[9]}.

The increasing momentum around \texttt{io\_uring} has drawn more eyes to the system, leading to more features being added \cslcitation{10}{[10]}, and vulnerabilities being patched \cslcitation{11}{[11]}, \cslcitation{12}{[12]}.
\section{Asynchronous IO}
\label{sec:orgbf1de1a}
Most if not all system calls used in day to day software are considered ``blocking'', meaning they will only return once the entire function has finished executing. This model allows for programs to not having to worry about order of execution, as they know their program will run top to bottom without skipping a step. This paradigm was formed under the (correct) assumption that IO was always the bottleneck of an application, however, modern systems are now at the point where CPU is once again becoming a limiting factor \cslcitation{13}{[13]}, \cslcitation{14}{[14]}.

To avoid the performance impact of synchronous IO, programmers may choose to design their software around asynchronous IO. For most applications it may not make sense to perform all system calls concurrently/asyncronously \cslcitation{15}{[15]}. We certainly want slower system calls to complete in the background, but the added overhead of an asynchronous system call may make faster calls worse. We expect to see greater differences in throughput for large files as they would stall the server while attempting to read/write data that may not fit in a single block/page.
\section{Latency}
\label{sec:orgc379194}
A common mark of server performance is requests per second \cslcitation{1}{[1]}, \cslcitation{2}{[2]}, but this metric only applies to some workouts. If the goal is to server many concurrent connections without massive latency, special care needs to be taken to how requests are handled. Specifically for a file server, the application needs to ensure that requests are served as soon as the information is available, not stalling for large IO operations \cslcitation{16}{[16]}.

This problems was first described as the ``C10K'' problem, which refers to handling 10,000 concurrent requests on a server in 1999. The discussions at the time covered different server architectures, ranging from fully synchronous, to the state of the art, which at the time was the emerging \texttt{aio} system. A general conclusion is that the scalability of web servers is largely dependent on how request handling is scheduled, and how many tasks can be performed concurrently \cslcitation{3}{[3]}.

Both \texttt{poll} and \texttt{epoll} (along with other synchronous approaches) fall short in this regard. Due to the linear scanning of all incoming requests, latency will grow quickly as the number of clients increases (figure X)\footnote{This needs a figure citation!}. I threaded model will easily outperform a single core are lower connected clients, but will quickly begin to climb in latency as the number of clients grows. The overhead from spawning a thread per request is not feasible past a certain point. \texttt{io\_uring} is able to avoid this problem by scheduling requests within the kernel, which allows the application to handle an event as soon as it completes \cslcitation{4}{[4]}.
\section{\texttt{aio}}
\label{sec:orgafbb04a}
...
\texttt{aio} is also very limited in scope, only offering real support for read and write calls \cslcitation{4}{[4]}. This means that without splicing together features from different asynchronous IO frameworks an application will still have to rely heavily on blocking system calls \cslcitation{10}{[10]}.

For all of these reasons \texttt{aio} is generally avoided in application code.
\section{\texttt{epoll}}
\label{sec:orge719657}
...
\label{sec:org43d7607}
\texttt{io\_uring} is the latest attempt at adding asynchronous operations to Linux. Its design makes it obvious that it the designer learned from many of the shortcomings of \texttt{aio}. Not only does \texttt{io\_uring} provide a common interface across all types of file descriptors, it also implements most systems calls in an asyncronous fashion \cslcitation{4}{[4]}, \cslcitation{5}{[5]}, \cslcitation{10}{[10]}.

The design can be broken down into two distinct parts, a job submission queue, and a job completion queue, both implemented via ring buffers shared between the kernel and user space. At a basic level each submission is a combination of an op code defining which system call to perform, and the associated arguments \cslcitation{4}{[4]}. If the user desires to keep track of a job they can associate user data with a submission, which takes the form of an 64-bit integer, commonly used to hold a pointer. The implementation garuntees that user data will not be modified.
...
\cslbibitem{7}{\cslleftmargin{[7]}\cslrightinline{D. Didona, J. Pfefferle, N. Ioannou, B. Metzler, and A. Trivedi, “Understanding modern storage apis: a systematic study of libaio, spdk, and io\_uring,” in \textit{Proceedings of the 15th acm international conference on systems and storage}, 2022, pp. 120–127.}}

\cslbibitem{8}{\cslleftmargin{[8]}\cslrightinline{Z. Ren and A. Trivedi, “Performance characterization of modern storage stacks: Posix i/o, libaio, spdk, and io\_uring,” in \textit{Proceedings of the 3rd workshop on challenges and opportunities of efficient and performant storage systems}, 2023, pp. 35–45.}}

\cslbibitem{9}{\cslleftmargin{[9]}\cslrightinline{T. Endo and S. M. S. Al-Mashni, “Comparative evaluation of asynchronous io interface between io\_uring and libaio implemented in a nosql db for ssds,” \textit{情報処理学会第 82 回全国大会}, vol. 5, p. 02, 2020.}}

\cslbibitem{10}{\cslleftmargin{[10]}\cslrightinline{J. Axboe, “What’s new with io\\_uring,” \textit{Kernel recipes}, 2022.}}

\cslbibitem{11}{\cslleftmargin{[11]}\cslrightinline{W. He, H. Lu, F. Zhang, and S. Wang, “Ringguard: Guard io\_uring with ebpf,” in \textit{Proceedings of the 1st workshop on ebpf and kernel extensions}, 2023, pp. 56–62.}}

\cslbibitem{12}{\cslleftmargin{[12]}\cslrightinline{J. Xu \textit{et al.}, “Mock: Optimizing kernel fuzzing mutation with context-aware dependency.”}}

\cslbibitem{13}{\cslleftmargin{[13]}\cslrightinline{P. Enberg, A. Rao, and S. Tarkoma, “I/o is faster than the cpu: Let’s partition resources and eliminate (most) os abstractions,” in \textit{Proceedings of the workshop on hot topics in operating systems}, 2019, pp. 81–87.}}

\cslbibitem{14}{\cslleftmargin{[14]}\cslrightinline{Y. Zhong \textit{et al.}, “Bpf for storage: an exokernel-inspired approach,” in \textit{Proceedings of the workshop on hot topics in operating systems}, 2021, pp. 128–135.}}

\cslbibitem{15}{\cslleftmargin{[15]}\cslrightinline{J. Ousterhout, “Why threads are a bad idea (for most purposes).” Talk, Sep. 1995.}}
